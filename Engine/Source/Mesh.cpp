...
Mesh::Mesh(std::vector<Vertex> vert, std::vector<unsigned int> ind, std::vector<Texture> text, std::vector<float2> texCoord) : vertices(vert), indices(ind), textures(text), texCoords(texCoord), vao(0), vbo(0), ebo(0)
{
	SetupMesh();
}
