...
public class Boss : RagnarComponent
{
	...
	private bool PerceptionCone(int angleDegrees)
	{
		...
		index = RayCast.PerceptionCone(initPos, bossForward, angleDegrees, 16, 8, players, players.Length, colliders, colliders.Length, Time.deltaTime);
	...
	{
		if (players[0] != null && Math.Abs((players[0].transform.globalPosition - gameObject.transform.globalPosition).magnitude) <= 1.0f)
		{
			// Play sweep attack animation
